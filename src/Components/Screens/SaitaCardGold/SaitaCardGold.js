...
import { styles } from './SaitaCardGoldStyle';

const windowHeight = Dimensions.get('window').height;
const windowWidth = Dimensions.get('window').width;
let ratioHeight = (362 / 228) * 228;

const SaitaCardGold = props => {
  ...
  useEffect(() => {
    //console.warn('MM','>>>>>>> goldddddd screen', props?.item);
    getDataCard();
  }, []);
  ...
  const getUserDetail = access_token => {
    return new Promise((resolve, reject) => {
      ...
      dispatch(getUserCardDetail({access_token}))
        .then(res => {
          //  console.warn('MM','getUserCardDetail gold:::::::', res);
          setUserDetail(res);
          const selectedData = res.cards.find(
            el => el.card_table_id == props.item.card_table_id,
          );
          //  console.warn('MM','selectedData gold:::::::', selectedData);
          // if (res.kyc_status == 2) {
  ...
  const getCardDetail = (access_token, btnPressed = false, details = false) => {
    ...
    dispatch(getCardsDetailApi({data, access_token}))
      .then(res => {
        setisLoading(false);
        //  console.warn('MM','Card Detail Res:::::::::gold', res);
        if (res.data) {
          const data = res.data;
            ...
            if (data.fee_status == 'complete') {
              if (data.kyc_status == 0) {
                ...
                !isActiveBlack &&
                  ...
                  navigate(NavigationStrings.SaitaCardApplyForm,{cardType: 3, selectedItem: data});
                return;
              } else if (data.kyc_status == 2 && data.card_applied == 0) {
                return Singleton.showAlert(
                  'Your Saitacard Gold is applied successfully and is under Approval.We will inform you the Approval via an email.',
                );
              }
            } else {
  ...
  const createWalletAddress = req => {
    // console.warn('MM','_________________', req);
    let data = {
      ...
      <TouchableOpacity
        ...
        <View style={styles.balanceViewStylehistory}>
          <Text
            ...
            {/* {Singleton.getInstance().toFixed(item.item.tx_amount, 2)}{' '}
            {item.item.tx_currency.toUpperCase()} */}
          ...
          <Text
            style={[styles.coinPriceStylehistory, {color: Colors.lightGrey2}]}>
            {/* {Singleton.getInstance().toFixed(item.item.fee, 2)} */}
            {/* {new Date((item.item.transaction_date - 12600 )* 1000).toLocaleDateString()} */}
      ...
      <Wrap style={{backgroundColor: ThemeManager.colors.backgroundColor}}>
        ...
        <View
          ...
          <TouchableOpacity
            ...
            <Text
              style={[styles.login, {color: ThemeManager.colors.textColor}]}>
              Card Features
            </Text>
          </TouchableOpacity>
        </View>
  ...
  const hidePressed = async () => {
    ...
    setTimeout(() => {
      ...
      if (hideView == true) {
        ...
        Singleton.showAlert('hideee111 else');
      }
    }, 100);
  };
  const activeCard = () => {
