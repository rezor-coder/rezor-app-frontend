...
class SendSTC extends Component {
  ...
  componentDidMount() {
    ...
    Singleton.getInstance()
      ...
      .then(stcPvtKey => {
        this.setState({ stcPvtKey: stcPvtKey });
      });
  ...
  slowAction() {
    let totalFee = this.state.gasPriceForTxnSlow *
      this.state.gaslimitForTxn *
      this.state.gasFeeMultiplier

    if (
      parseFloat(+this.state.walletData.balance - (totalFee + 0.005)) <= 0
    ) {
    ...
    if (this.state.isMaxClicked) {
      let newamount = Singleton.getInstance().toFixed(
        Singleton.getInstance().exponentialToDecimal(
          this.state.walletData.balance - (totalFee + 0.005)
        ),
        constants.CRYPTO_DECIMALS,
      )
  ...
  mediumAction() {
    let totalFee = this.state.gasPriceForTxnMedium *
      this.state.gaslimitForTxn *
      this.state.gasFeeMultiplier
  ...
  highAction() {
    let totalFee = this.state.gasPriceForTxnHigh *
      this.state.gaslimitForTxn *
      this.state.gasFeeMultiplier
  ...
  getGasLimit() {
    ...
    let data = {
      ...
      to: Singleton.getInstance().defaultStcAddress,
      amount: this.state.amount,
    };
    let blockChain = this.state.blockChain;
    let access_token = Singleton.getInstance().access_token;
    let contractAddress =
      this.state.walletData.coin_symbol.toLowerCase() == 'stc'
        ? 'stc'
      ...
      this.props
        ...
        .then(response => {
          ...
          if (response.status) {
            ...
            console.log("slowGasPrice::::", slowGasPrice, "mediumGasPrice:::::", mediumGasPrice, "heightGasPrice::::", heightGasPrice);
            let feeIs =
              mediumGasPrice *
  ...
  onSendAction() {
    ...
    console.warn('MM', 'this.state.walletData ', this.state.walletData?.balance, this.state.amount);

    if (this.state.walletData.is_token == 0) {
      if (
        parseFloat(+this.state.walletData?.balance) < parseFloat(+this.state.amount + (+this.state.totalFee + 0.005))
      ) {
  ...
  findMaxSend() {
    if (this.state.maxClicked) {
      console.warn('MM', 'this.state.totalFee', this.state.totalFee, this.state.walletData?.balance - (+this.state.totalFee + 0.05));
      if (this.state.walletData?.coin_symbol?.toLowerCase() == 'stc') {
        if (
          parseFloat(+this.state.walletData.balance - (+this.state.totalFee + 0.005)) <= 0
        ) {
          Singleton.showAlert(LanguageManager.lowBalanceAlert);
          this.setState({ maxClicked: false });
          return;
        ...
        this.setState({
          amount: Singleton.getInstance()
            .toFixednew(
              +this.state.walletData?.balance - (+this.state.totalFee + 0.005),
              8,
            )
            .toString(),
          isMaxClicked: true
  ...
  send_STC() {
    ...
    let chainId = IS_PRODUCTION == 0 ? 129 : 1209;
    getStcNonce(wallet_address)
      .then(nonce => {
        console.warn('MM', 'Chk stc nonce::::::::', nonce);
        getStcRaw(
          Singleton.getInstance().toFixednew(this.state.amount, 8),
          this.state.to_Address,
          nonce,
          this.state.gasPriceForTxn,
          this.state.gaslimitForTxn,
          chainId,
          this.state.stcPvtKey,
        )
          .then(stcSignedRaw => {
            console.warn('MM', 'Chk StcSignedRaw::::::::', stcSignedRaw);
            this.send(
              stcSignedRaw,
          ...
          .catch(err => {
            this.setState({ isLoading: false });
            console.warn('MM', 'Chk StcSignedRaw err::::::::', err);
          });
      })
      .catch(err => {
        this.setState({ isLoading: false });
      ...
      .then(res => {
        ...
        this.props.CheckIsContactExist({ data: req, access_token }).then(response => {
          this.setState({
            ...
            pinModal: false,
            BasicModall: true,
            showAddContact: response.is_contact == 1 ? false : true
          });
          setTimeout(() => {
            this.setState({ isLoading: false });
          }, 150);
              ...
              <View style={{ marginHorizontal: widthDimen(22) }}>
                ...
                <View>
                  <BasicInputBox
                    ...
                    onChangeText={text => {
                      this.setState({ isMaxClicked: false })
              ...
              <View style={{ marginTop: heightDimen(15) }}>
                <View style={styles.transaction__options}>
                  <Text
                    ...
                    {LanguageManager.transactionFeesSTC}{' '}
                  </Text>
                </View>
                    ...
                    <Text
                      ...
                      ]}>
                      {
                        this.state.amount
                          ...
                          : Singleton.getInstance().toFixednew(this.state.totalFee, 8)}{' '}
                      {this.state.walletData?.coin_symbol?.toUpperCase()}
                    </Text>
                  ) : (
            ...
            <Wrap style={{ backgroundColor: ThemeManager.colors.bg }}>
              ...
              <View style={{ flex: 1, marginHorizontal: widthDimen(22) }}>
                ...
                <DetailOption
                  type={'From'}
                  item={'From'}
                  value={Singleton.getInstance().defaultStcAddress}
                  bottomLine={true}
                />
