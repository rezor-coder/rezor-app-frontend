import { StyleSheet, Dimensions } from 'react-native';
import { Images, Colors, Fonts } from '../../../theme';
import fonts from '../../../theme/Fonts';
import { LanguageManager, ThemeManager } from '../../../../ThemeManager';
import { windowWidth } from '../../../Constant';
import { areaDimen, heightDimen, widthDimen } from '../../../Utils/themeUtils';
const win = Dimensions.get('window').width;
const imgw = win - 180


export const styles = StyleSheet.create({
    BYsaita: {
        height: 33,
        width: 30,

        resizeMode: 'contain',

        alignSelf: 'center',
    },
    depositButton: {
        marginTop: heightDimen(20),
        height: heightDimen(40),
        width: widthDimen(150),
        borderRadius:100,
        backgroundColor:Colors.white,
        justifyContent:'center',
        alignItems:'center',
        flexDirection:'row'
    },
    inputText: {
        fontSize: 20,
        // color: '#fff',
    },
    cellStyle: {
        borderWidth: 1,
        borderRadius: 12,
        // width: 48,
        // height: 55,
        borderColor: Colors.languageItem,
        marginLeft: 10,
    },
    ViewStyle: {
        marginLeft: 10, alignItems: 'center', flexDirection: 'row'
    },
    ViewStyle1: {
        marginLeft: 10, alignItems: 'center', flexDirection: 'row',
    },
    eyeStyle: {
        position: 'absolute',
        alignSelf: 'flex-end',
        top: heightDimen(15),
        right: widthDimen(15),
        zIndex: 1,
        // padding: 5
    },
    eyeStyleNew: {
        position: 'absolute',
        alignSelf: 'flex-end',
        top: 40,
        right: 45,
        zIndex: 1,
        padding: 8
    },
    activeCardName: {
        // backgroundColor: 'rgba(57, 57, 57, 0.7)',
        borderRadius: 17,
        height: 150,
        // width: 230,
        justifyContent: 'center',
        paddingRight: widthDimen(20),
    },
    textStylee: {
        fontSize: 14,
        fontFamily: Fonts.regular,
        color: ThemeManager.colors.stakeColor,
        textTransform: 'capitalize'
    },
    centerLottie: {
        // height: 280,
        // width: 380,
        // resizeMode: 'stretch',
        // alignSelf: 'center',
    },

    container: {
        flex: 1,
        // backgroundColor: Colors.black,
    },
    flatList: {
        backgroundColor: ThemeManager.colors.defiBgColor,
        width: '90%',
        alignSelf: 'center',
        marginVertical: 20,
        borderRadius: 10,
    },
    backTouchable: { marginTop: 44, marginStart: 12, padding: 5 },

    imgBackStyle: { width: 18, height: 18, resizeMode: 'contain' },

    imgSaitaPro: {
        height: 60,
        width: 140,
        resizeMode: 'contain',
        alignSelf: 'center',
    },
    imgcards: {
        height: heightDimen(494), width: widthDimen(212), borderRadius: 25, overflow: 'hidden', alignItems: 'center', justifyContent: 'center',
        alignSelf: 'center', marginTop: heightDimen(59.5)
    },

    txtone: {
        color: Colors.White,
        fontFamily: fonts.semibold,
        fontSize: 8,
    },
    InaCtiveText: {
        color: Colors.red_dark,
        fontFamily: fonts.semibold,
        fontSize: 14,
        textDecorationLine: 'underline'
    },
    cardName: {
        color: Colors.White,
        fontFamily: fonts.bold,
        fontSize: 14,
        textTransform: 'capitalize'
    },
    txttwo: {
        color: "#8F939E",
        fontFamily: fonts.bold,
        fontSize: 14, textDecorationLine: 'line-through',
        textDecorationStyle: 'solid'
    },
    txtthree: {

        color: Colors.White,
        fontFamily: fonts.bold,
        fontSize: 16,
    },

    txtdescribe: {
        color: ThemeManager.colors.lightTextColor,
        fontFamily: fonts.semibold,
        fontSize: 16,
        alignSelf: 'center',
        textAlign: 'center',
        paddingHorizontal: 38,
        marginTop: 28,
    },
    login: {

        fontFamily: fonts.medium,
        fontSize: 15,
        alignSelf: 'center',
        textAlign: 'center',
        marginTop: 22
    },

    btnStyle: {
        width: '100%',
        height: widthDimen(60),
        // marginTop: 35,
        paddingHorizontal: 32,
        marginTop: 30,
        borderRadius: widthDimen(30),
    },
    disableBtnStyle: {
        width: widthDimen(370),
        height: widthDimen(60),
        marginTop: heightDimen(97),
        backgroundColor: ThemeManager.colors.backgroundColor,
        borderWidth: 1, borderColor: ThemeManager.colors.viewBorderColor,
        borderRadius: 120,
        justifyContent: 'center'
    },
    customGrad: {
        borderRadius: widthDimen(30),
    },

    txtImportWallet: {
        // color: Colors.White,
        fontFamily: fonts.semibold,
        fontSize: 15,
        alignSelf: 'center',
        textAlign: 'center',
        marginTop: 10,
        padding: 5,
        marginBottom: 40,
    },










    txtlist: {

        fontFamily: fonts.semibold,
        fontSize: 16,
    },
    tokenlistActive: {
        fontFamily: fonts.semibold,
        fontSize: areaDimen(16),
        lineHeight:heightDimen(19),
        
    },
    tokenlistInActive: {
        color: '#6E737E',
        fontFamily: fonts.regular,
        fontSize: 12,
    },
    txtviewall: {
        fontFamily: fonts.semibold,
        fontSize: areaDimen(14),
        lineHeight:heightDimen(18),
        alignSelf: 'flex-end'
    },

    txtonecard: {
        marginTop: heightDimen(30),
        fontFamily: fonts.medium,
        fontSize: areaDimen(14),
        lineHeight: heightDimen(18),
        alignSelf: 'center',
    },
    txttwocard: {
        alignSelf: 'center',
        fontFamily: fonts.semibold,
        fontSize: areaDimen(24),
        lineHeight: heightDimen(29),
        marginTop: heightDimen(10)
    },
    txtthreecard: {

        color: Colors.lightGrey2,
        fontFamily: fonts.regular,
        fontSize: 14, marginTop: 12
    },

    txtcvv: {
        color: "rgba(255, 255, 255, 0.5)",
        fontFamily: fonts.regular,
        fontSize: areaDimen(6),
    },
    txtdate: {
        color: Colors.white,
        fontFamily: fonts.regular,
        fontSize: areaDimen(12),
        marginTop: 3,
        letterSpacing: 1
    },

    txtdescribe: {
        color: ThemeManager.colors.lightTextColor,
        fontFamily: fonts.medium,
        fontSize: areaDimen(16),
        lineHeight: heightDimen(18),
        alignSelf: 'center',
        textAlign: 'center',
        // paddingHorizontal: 38,
        // marginTop: 10
    },
    login: {
        color: ThemeManager.colors.textColor,
        fontFamily: fonts.medium,
        fontSize: 15,
        alignSelf: 'center',
        textAlign: 'center',
        marginTop: 15
    },

    btnStylecard: {

        marginTop: 10,
    },

    customGradCard: {
        borderRadius: 8,
        height: 30, width: 70
    },

    txtImportWallet: {
        // color: Colors.White,
        fontFamily: fonts.semibold,
        fontSize: 15,
        alignSelf: 'center',
        textAlign: 'center',
        marginTop: 10,
        padding: 5,
        marginBottom: 40,
    },
    txtCardone: {
        color: Colors.white,
        fontFamily: fonts.semibold,
        fontSize: areaDimen(18),
        textTransform: 'capitalize',
        letterSpacing: 1
      },
      txtCardred: {
        color: ThemeManager.colors.textColor,
        fontFamily: fonts.medium,
        fontSize: areaDimen(10),
      },
    
    txtCardthree: {

        color: Colors.lightGrey2,
        fontFamily: fonts.regular,
        fontSize: 14,
    },

    txtstar: {

        color: Colors.White,
        fontFamily: fonts.regular,
        fontSize: areaDimen(11),
        paddingVertical: 10,
        letterSpacing: 1
    },
    coinsListStyle: {
        flex: 1,
        flexDirection: 'row',
        padding: 10,
        marginHorizontal: -15,
        paddingHorizontal:widthDimen(21.5),
        alignItems: 'center',
        alignItems: 'center',
        borderRadius: 10,
        justifyContent: 'space-between',

    },
    balanceViewStyle: {
        alignSelf: 'flex-end',
        width: '28%',
        backgroundColor: 'blue'
    },
    balanceViewStylehistory: {
        alignItems: 'flex-end'
    },
    balanceTexthistory: {
        fontFamily: Fonts.medium,
        fontSize: areaDimen(14),
    },
    coinPriceStylehistory: {
        fontSize:areaDimen(12),
        fontFamily: Fonts.medium,
    },
    balanceText: {
        alignSelf: 'flex-end',
        fontFamily: Fonts.regular,
        textAlign: 'right',
        fontSize: 13,
    },
    coinStyle: {
        height: heightDimen(13.5),
        width: widthDimen(10.5),
        resizeMode:'contain'
    },
    coinPriceStyle: {
        alignSelf: 'flex-end',
        fontSize: 11,
        fontFamily: Fonts.regular,
    },
    graphImage: {
        height: 40,
        width: 150,
        resizeMode: 'contain',
    },
    coinListWrapStyle: {
        marginTop: 15,
        paddingBottom: 70,
        flexGrow: 1,
        marginHorizontal: 10
    },
    noCoinText: {
        color: ThemeManager.colors.textColor,
        textAlign: 'center',
        marginTop: 30,
        fontSize: 16,
    },
    coinNameStyle: {
        height: 30,
        width: 30,
        backgroundColor: Colors.buttonColor2,
        alignItems: 'center',
        justifyContent: 'center',
        borderRadius: 40,
    },
    fiatText: {
        color: Colors.lightGrey2,
        fontFamily: Fonts.medium,
        fontSize: areaDimen(12),
        marginLeft: 1,
        textAlign: 'center',
        textTransform: 'capitalize',
        marginTop:heightDimen(4)
    },
    graphStyle: {
        width: '30%',
        marginRight: 10,
        alignSelf: 'flex-start',
    },
    manageBtnStyle: {
        justifyContent: 'center',
        alignItems: 'center',
        flexDirection: 'row',
    },
    manageBtnImg: {
        height: 14,
        width: 14,
        resizeMode: 'contain',
        tintColor: ThemeManager.colors.textColor,
        marginRight: 5,
    },
    assetsText: {
        color: ThemeManager.colors.textColor,
        fontSize: 16,
        fontFamily: fonts.semibold,
    },
    subView: {
        flexDirection: 'row',
        justifyContent: 'space-between',
        marginTop: 15,
        marginHorizontal: 15
    },
    subContainer: {
        width: '100%',
        marginTop: 20,
        borderRadius: 10,
        justifyContent: 'center',
        paddingVertical: 5,
    },
    flexRow: { flexDirection: 'row', justifyContent: 'space-evenly' },
    okText: {
        color: Colors.white,
        fontSize: 16,
        fontFamily: fonts.regular,
        // marginTop:-2,
        marginTop: 2
    },
    refreshBtn: {
        alignSelf: 'flex-end',
        marginTop: -30,
        marginRight: -10,
    },
    txtWelcome: {
        fontFamily: fonts.semibold,
        fontSize: areaDimen(20),
        alignSelf: 'center',
        textAlign: 'center',
    },
    txtkyc: {
        fontFamily: fonts.regular,
        fontSize: areaDimen(14),
        color: "#989898",
        alignSelf: 'center',
        textAlign: 'center',
        marginTop: heightDimen(10),
        lineHeight: heightDimen(24),
    },
    customGrad: {
        borderRadius: 100,
        marginTop: heightDimen(97),
        alignSelf:'center'
    },
    btnStylekyc: {
        width: widthDimen(370),
        height: 50,
      
    },
    normalBg: {
        backgroundColor: ThemeManager.colors.defiBgColor,
        justifyContent: 'center',
        alignItems: 'center',
        flex: 1,
        height: 50,
        borderRadius: 10,
    }, imgCopyInside: {
        height: heightDimen(15),
        width: heightDimen(15),
        // marginLeft:5
    },
});
