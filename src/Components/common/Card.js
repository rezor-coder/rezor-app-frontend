...
const Card = props => {
  return props.isSaitaDex ? (
    <View
      style={[styles.ViewStyle, {backgroundColor: ThemeManager.colors.iconBg}]}>
      ...
      <View style={{marginLeft: widthDimen(12), flex: 1}}>
        <Text
          style={[{color: ThemeManager.colors.textColor}, styles.textStyle]}>
          {props.item?.coin_name?.toUpperCase() +
            ' (' +
      ...
      <View style={{flexDirection: 'row'}}>
        <TouchableOpacity
          onPress={() => props.onPressSwap(props.item)}
          style={[
            styles.touchableStyle,
            {backgroundColor: ThemeManager.colors.primary},
          ]}>
          <Text style={[{color: Colors.white}, styles.textStyle2]}>
    ...
    <View
      ...
      <View style={styles.ViewStyle1}>
        <Text style={styles.textStyle3}>
          {props.item?.receivingCurrency.charAt(0)}
        </Text>
      </View>
      <View style={{marginLeft: widthDimen(12), flex: 1}}>
        <Text
          style={[{color: ThemeManager.colors.textColor}, styles.textStyle]}>
          {props.item?.receivingCurrency?.toUpperCase() +
            '/' +
            props.item?.receivableCurrency?.toUpperCase()}
        </Text>
        <Text
          style={[
          ...
          numberOfLines={1}>
          $
          {props.item?.price >= 0.01
            ? CommaSeprator3(props.item?.price, 2)
            : CommaSeprator3(
                Singleton.getInstance().exponentialToDecimal(props.item?.price),
                8,
              )}
        </Text>
      </View>
